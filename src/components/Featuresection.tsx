...
const Featuresection = () => {
  return (
    <div 
      id="features"
      className="w-full min-h-[100dvh] bg-gradient-to-b from-black to-[#52057B] flex flex-col"
    >
      <div className="container mx-auto flex flex-col flex-grow py-20 px-4">
        {/* Heading */}
        <h1 className="text-white font-poppins text-center text-[35px] sm:text-[38px] md:text-[55px]">
          Why Choose an{" "}
          <span className="text-[#540E92] font-alkatra">Ai-Powered Resume?</span>
        </h1>

        {/* Cards - Simplified grid layout */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mt-20 w-full">
          {[
            { title: "Ai Resume Optimization", text: "Enhance your resume...", icon: chip },
            { title: "AI Resume Builder", text: "Create a professional...", icon: aifile },
            { title: "ATS Score Checker", text: "Analyze your resume's...", icon: filecheck }
          ].map((feature, index) => (
            <div 
              key={index}
              className="bg-black bg-opacity-95 border-2 border-[#040404] rounded-[15px] p-6 flex flex-col items-center hover:scale-105 hover:border-[#8A2BE2] transition-all"
            >
              <p className="text-[28px] text-[#CCCCCC] text-center">{feature.title}</p>
              <p className="text-[13px] text-[#CCCCCC] mt-4 text-center">{feature.text}</p>
              <Image src={feature.icon} alt="" className="mt-6" />
            </div>
          ))}
        </div>
      </div>
